...
contract Semaphore is IWorldID, SemaphoreCore, Verifier, SemaphoreGroups {
    ...
    function checkValidRoot(
        uint256 groupId,
        uint256 root
    ) public view returns (bool) {
        if (
            root != latestRoots[groupId] || latestRoots[groupId] == EMPTY_LEAF
        ) {
            RootHistory memory rootData = rootHistory[root];

            if (block.timestamp - rootData.timestamp > ROOT_HISTORY_EXPIRY)
                revert ExpiredRoot();

            if (rootData.groupId == 0 && rootData.timestamp == 0)
